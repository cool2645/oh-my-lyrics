...
import { Extension, Lyrics, Sentence, Word } from '@/model'
...
export const mutations = {
  updateField,
  updateCurrentDocumentField (state: State, field: string) {
  ...
  OPEN_TAB (state: State, payload: { tab: Tab, id: number }) {
    state.tabs.splice(payload.id, 0, payload.tab)
    state.currentTabId = payload.id
  },
  CLOSE_TAB (state: State, id: number) {
  ...
  ACTIVATE_TAB (state: State, id: number) {
    state.currentTabId = id
  },
  REORDER_TAB (state: State, direction: number) {
  ...
  UPDATE_EDITOR_MODE (state: State, { editorMode, id }: { editorMode: EditorMode, id: number }) {
