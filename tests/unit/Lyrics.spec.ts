...
const { LYRICS_MUTATION } = mutations

const ORIGINAL_STATE = JSON.stringify({
  'font': 'Serif',
  'tabs': [{
...
const ORIGINAL_STATE2 = JSON.stringify({
  'font': 'Serif',
  'tabs': [{
    '_id': '15744965228470.16697444522669425',
    'editorMode': 'LYRICS',
    ...
    'document': {
      ...
      'sentences': [{
        ...
        'words': [{ 'phonemes': [{ 'value': '崭' }], 'rubies': [] }, {
          ...
          'phonemes': [{ 'value': '伴' }],
          'rubies': []
        }, { 'phonemes': [{ 'value': '岁' }, { 'value': '月' }], 'rubies': [] }, {
          'phonemes': [{ 'value': '正' }],
          'rubies': []
        }, {
          'phonemes': [{ 'value': '等' }, { 'value': '待' }, { 'value': '着' }],
...
describe('LYRICS_MUTATION', () => {
  it('should delete phonemes', () => {
    ...
    expect(state).toStrictEqual(JSON.parse(STATE1))
  })
  it('should delete words', () => {
    ...
    expect(state).toStrictEqual(JSON.parse(STATE2))
  })
  it('should append', () => {
    const state = JSON.parse(STATE3)
    LYRICS_MUTATION(state, {
      from: { line: 6, ch: 20 },
      to: { line: 6, ch: 20 },
      text: ['……']
    })
    expect(state).toStrictEqual(JSON.parse(STATE4))
  })
  it('should go next line', () => {
    const state = JSON.parse(STATE4)
    LYRICS_MUTATION(state, {
      from: { line: 6, ch: 22 },
      to: { line: 6, ch: 22 },
      text: ['', '']
    })
    expect(state).toStrictEqual(JSON.parse(STATE5))
  })
  ...
  it('should break phoneme 1', () => {
    ...
    expect(state).toStrictEqual(JSON.parse(STATE3_1))
  })
  it('should break phoneme 2', () => {
    const state = JSON.parse(ORIGINAL_STATE2)
    LYRICS_MUTATION(state, {
      from: { line: 2, ch: 7 },
      to: { line: 2, ch: 9 },
